...
func TestIdentityJWTVerifierRejectsWrongAudienceAndLongLifetime(t *testing.T) {
	...
	for name, mutate := range map[string]func(map[string]any){
		...
		t.Run(name, func(t *testing.T) {
			claims := map[string]any{}; for k,v := range base { claims[k]=v }; mutate(claims)
			req := httptest.NewRequest(http.MethodGet, "/", nil); req.Header.Set("Authorization", "Bearer "+signToken(t, privateKey, "kid-1", claims))
			if _, err := verifier.Verify(req); err == nil { t.Fatal("expected verification failure") }
		})
	}
}
...
func TestIdentityJWTVerifierRejectsServiceSubjectMismatch(t *testing.T) {
	...
	defer server.Close()
	now := time.Now().UTC().Truncate(time.Second)
	verifier := NewIdentityJWTVerifier(server.URL); verifier.Now = func() time.Time { return now }
	...
	req := httptest.NewRequest(http.MethodGet, "/", nil); req.Header.Set("Authorization", "Bearer "+signToken(t, privateKey, "kid-1", claims))
	if _, err := verifier.Verify(req); err == nil { t.Fatal("expected subject/service mismatch rejection") }
}
...
func signToken(t *testing.T, key *rsa.PrivateKey, kid string, claims map[string]any) string {
	...
	h := base64.RawURLEncoding.EncodeToString(header); p := base64.RawURLEncoding.EncodeToString(payload)
